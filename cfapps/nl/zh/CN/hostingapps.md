...
通过 {{site.data.keyword.Bluemix_notm}}，可以创建应用程序以及托管现有应用程序。只要您的应用程序是云就绪型应用程序，就可以将其迁移到 {{site.data.keyword.Bluemix_notm}} 中。 
...
您的应用程序如果符合以下所有原则，那么就是云就绪型应用程序，可迁移到 {{site.data.keyword.Bluemix_notm}} 中。如果违反了哪个原则，您通常可以通过修改您的应用程序来遵循该原则。
...
* 不要在应用程序中存储会话状态。
  ...
  如果无法完全消除会话状态的影响，请使用应用程序服务器外部的高可用性存储来存储会话状态。 
...
<li>通过运行以下命令来连接并登录到 {{site.data.keyword.Bluemix_notm}}。系统提示时，选择组织和空间。<pre>cf login -a https://api.chinabluemix.net</pre></li>
...
<li>可选：如果更改了应用程序，那么必须重新输入 cf push 命令来上传这些更改。cf 命令行界面会使用您先前的选项并根据您对提示的响应，用新的代码段来更新运行中的所有应用程序实例。</li>
</ol>
...
要在 {{site.data.keyword.Bluemix_notm}} 中运行代码，必须添加用于解析 VCAP_SERVICES 变量以获取有关服务连接的信息的代码逻辑。修改应用程序，以通过环境变量获取动态分配给服务实例的主机和端口。 
