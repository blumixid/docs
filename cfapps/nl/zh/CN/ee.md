...
构建、运行和部署应用程序时，可以使用 {{site.data.keyword.Bluemix_notm}} 用户界面、平台和一组工具。请按照以下步骤开始此端到端开发场景。
{:shortdesc}

## 注册
{: #ee_start}

开始之前，根据您的合同，您必须具有 {{site.data.keyword.Bluemix_notm}} 标识。在具有该标识后，登录到 {{site.data.keyword.Bluemix_notm}}。
...
在 {{site.data.keyword.Bluemix_notm}} 中，应用程序与组织和空间相关联。一个组织可由多个合作者拥有并使用。最初，您将获得根据您的用户名命名的缺省组织，并且您是唯一的合作者。您还将获得该组织内的一个空间。空间是运行应用程序的环境；例如，您可以拥有 dev 空间、test 空间和 production 空间，分别用作开发环境、测试环境和生产环境。 

对于此场景，应使用 Node.js 来开发 Web 应用程序。要创建应用程序，请执行以下步骤：
...
{{site.data.keyword.Bluemix_notm}} 提供了多个工具，供您在开始对应用程序进行编码时使用，例如 cf 命令行界面和 Eclipse 工具。您可以选择从 cf 命令行界面开始对应用程序 TestNode 进行编码。
  ...
  3. 安装 **cf** 命令行界面后，必须使用 **cf api** 命令来指定要使用的 {{site.data.keyword.Bluemix_notm}} 区域。**cf** 命令行界面使用 *https://api.Bluemix_URL*，其中 *Bluemix_URL* 是区域的 URL。输入以下命令，以连接到 {{site.data.keyword.Bluemix_notm}}：
...
{{site.data.keyword.Bluemix_notm}} 为大多数运行时框架和服务提供免费限额。除非使用量超过免费限额，否则 {{site.data.keyword.Bluemix_notm}} 不会对您进行收费。
