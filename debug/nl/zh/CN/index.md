...
如果您遇到 {{site.data.keyword.Bluemix_notm}} 问题，那么可以查看日志文件来调查问题并调试错误。
{:shortdesc}
...
根据部署新代码的方式，选择下列其中一个方法，来调试代码更改： 
  ...
  * 对于从 GUI（如 {{site.data.keyword.Bluemix_notm}} 用户界面）部署的新代码，您可以通过界面检查日志。例如，如果您从 {{site.data.keyword.Bluemix_notm}} 用户界面部署新代码，那么您可以转至“仪表板”、查找应用程序，然后查看日志以获取线索。有关如何从 {{site.data.keyword.Bluemix_notm}} 用户界面查看日志的更多信息，请参阅[从 Bluemix 仪表板查看日志](../monitor_log/monitoringandlogging.html#viewing_logs_UI){: new_window}。  
