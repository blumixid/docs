...
**注**：只有具有 Admin 许可权 (`ops.admin`) 的用户才可以安排和批准维护更新。
...
在“LDAP 配置详细信息”部分中，可以选择 LDAP 服务器，并查看有关用户和组映射的信息。 
...
- 资源使用情况信息，例如可用内存量和磁盘空间量、实际使用的内存量和磁盘空间量，以及所有 Droplet Execution Agent (DEA) 上的平均内存、磁盘和 CPU 使用量。要查看按 DEA 列出的内存、磁盘或 CPU 使用情况，请单击**细目**。您可以查看内存和磁盘的**保留**和**物理**的摘要。<dl>
	<dt><strong>物理</strong></dt>
	<dd>为环境购买的内存量或磁盘空间量。</dd>
	<dt><strong>保留</strong></dt>
	<dd>可保留用于运行环境中所有应用程序的内存量或磁盘空间量。由于在任何时候都不会使用运行应用程序所需的全部内存和磁盘空间，因此可以根据需要保留和使用这些资源。保留值大约是物理值数量的两倍。</dd>
	</dl>

	除了图形表示法外，还可以查看环境中使用的内存和磁盘空间的百分比。您也可以查看实际使用的保留量和物理量与可用的保留量和物理量的比较（以 GB 为单位）。要查看有关物理、保留和实际内存使用量的更多详细信息，请单击**历史记录**。可以将查看的时间范围指定为每周或每月。“历史内存使用情况”视图显示在所选时间内的内存使用情况图。  

	<dl>
	<dt><strong>保留限制</strong></dt>
	<dd>显示为水平虚线图，“保留限制”等于可保留用于运行环境中所有应用程序的内存总量。“保留限制”等于所购买物理内存量的两倍。</dd>
	<dt><strong>保留</strong></dt>
	<dd>“保留”条形图显示当前保留用于运行环境中所有应用程序的内存。要查看在特定时间点保留内存最多的组织，请单击“保留”条形图顶部与该时间点关联的点。然后，可以在生成的饼图中单击组织以查看有关该组织的更多信息。</dd>
	<dt><strong>物理限制</strong></dt>
	<dd>显示为水平虚线图，“物理限制”显示购买用于环境的物理内存量。</dd>
	<dt><strong>物理</strong></dt>
	<dd>“物理”条形图显示实际使用的内存量。</dd>
	</dl>
...
所有分配了具有**读**访问权的**用户**许可权的管理控制台用户，都可以查看帐户使用情况数据。此外，组织记帐管理员也可以查看其组织的帐户使用情况数据，即使没有为记帐管理员分配管理控制台**用户**许可权时也是如此。作为管理控制台管理员，您可以通过单击 **{{site.data.keyword.avatar}}** 图标 ![Avatar](../support/images/account_support.svg)，然后选择**管理组织**来更新组织的记帐管理员角色。
...
初始部署 {{site.data.keyword.Bluemix_notm}} 环境后，系统会对用于管理环境的组件自动完成验证检查。验证检查运行后，可以转至管理控制台的“验证检查”页面来检查组件的状态。要访问该页面，请转至 <code>https://console.&lt;subdomain&gt;.chinabluemix.net/check</code>，其中 `<subdomain>` 是您的本地或专用实例的名称。
...
- 环境内存使用情况。此部分详细描述了整个系统环境的内存使用情况。该图表提供的信息包括使用的物理内存和物理内存限制、保留的内存限制和保留内存的配额，以及环境的内存配额总量。以下术语列表定义了图表中显示的内存使用量的类型。
	...
	<dd>在所有组织中为所有部署的应用程序保留的内存总和。保留的配额总和可能会超过环境的物理内存限制。例如，如果物理内存限制为 16 GB，而您为共 5 个不同组织分别保留了 4 GB 的内存，那么保留的配额会超过物理内存限制。但是，在许多情况下，这些组织可能不会使用单独为每个组织保留的所有配额。此外，所有组织可能也不会同时使用保留的所有内存配额。</dd>
	<dt><strong>保留限制</strong></dt>
	<dd>可以为环境中的所有组织保留的内存总量。</dd>
	<dt><strong>配额总量</strong></dt>
	<dd>可以为环境中的所有组织分配的内存总量。</dd>
	</dl>
	**注**：数据每 4 小时自动刷新一次。如果要在页面上的数据自动更新之前刷新数据，请单击**重新计算**。

- 组织内存使用情况。此部分详细描述了组织级别的内存使用情况。可以查看配额限额总量和为每个组织保留的配额。该图表提供的信息按每个组织的最高内存保留量列出，缺省情况下，保留内存量最多的组织列在最前面。可以按**最高内存使用量**和**多余内存分配量**进行排序。

	<dl>
	<dt><strong>最高内存使用量</strong></dt>
	<dd>使用此选项可识别内存使用量最高的组织。按最高内存使用量排序可识别使用内存量最多的组织。此列表按部署的配额进行排序。</dd>
	<dt><strong>多余内存分配量</strong></dt>
	<dd>使用此选项可识别保留的配额大于需求的组织。按多余内存分配量排序可识别在为组织分配的配额中所使用的内存量最低的组织。</dd>
	</dl>
...
* 查找用户。如果您具有 **Read** 和 **Write** 访问权且您了解所有或部分用户名，那么您可以使用**搜索**字段，在表中查找用户。您还可以按用户的组织和许可权来过滤用户列表。
   <ol>
  <li> 要过滤用户列表，请单击<strong>过滤</strong>。</li>
  <li> 单击<strong>组织</strong>或<strong>许可权</strong>。<dl>
	<dt><strong>组织</strong></dt>
	<dd>要按用户的组织过滤用户，请在“组织”字段中开始输入组织名称，并从列表中选择相应组织。然后，选择在该组织内分配给该用户的一个或多个角色。</dd>
	<dt><strong>许可权</strong></dt>
	<dd>要按用户的许可权过滤用户，请首先选择一个或多个用户的类型。例如，您可能希望查看所有 Superuser。对于除 Superuser 或 Basic Access 以外的其他许可权，还可以选择访问权类型，例如<strong>读</strong>或<strong>写</strong>。</dd>
	</dl></li>
  <li>单击<strong>应用</strong>。</li>
   </ol>  
   “用户管理”窗口在生成的表中显示设置的过滤器和用户。然后，可以在过滤后的表中搜索用户。还可以通过单击以关闭列表中的过滤器来除去过滤器。
...
要登录到管理控制台，可以在 `https://login.chinabluemix.net/UAALoginServerWAR/nglogin.jsp` 端点上使用基本访问认证。服务器会使用您的会话返回 cookie。您使用该 cookie 来执行所有管理控制台操作。
...
所有 API 都是相对于 <code>https://console.&lt;subdomain&gt;.chinabluemix.net/</code> 的。

<dl>
<dt><strong>&lt;subdomain&gt;</strong></dt>
<dd>此值是您本地或专用实例的名称。{{site.data.keyword.Bluemix_notm}} 实例的子域名称已在上线时进行了分配。</dd>
</dl>
